...
class StockLocation(models.Model):
    ...
    def create(self, vals):
        ...
        if (
            ...
            and stock_location.is_magento_source == True
            and magento_backend != False
        ):
            try:
                params = {
                    "source": {
                ...
                client = Client(
                    magento_backend.web_url, magento_backend.access_token, True
                )
        ...
        for e in stock_quant:
            if e.product_id.on_magento == True:
                try:
                    ...
                    raise UserError(
                        ("Can not update quantity product on source magento - %s")
                        % tools.ustr(a)
                    )
            ...
            if "active" not in vals:
                ...
                if self.usage == "internal" and self.is_magento_source == True:
                    try:
                        ...
                        raise UserError(
                            ("Can not write source to magento - %s") % tools.ustr(e)
                        )
                    self.update_quantity_product_location(self.id, magento_backend)
                elif (
                    self.usage == "internal"
                    and self.is_magento_source == False
                    and "is_magento_source" in vals
                ):
                    try:
                        params = {
                            "source": {
                        ...
                        raise UserError(
                            ("Can not update source to magento - %s") % tools.ustr(e)
                        )
                elif (
                    self.usage == "internal"
                    and self.is_magento_source == False
                    and "is_magento_source" not in vals
                ):
                    return super(StockLocation, self).write(vals)
            ...
            if magento_backend == False:
                ...
                if rec.usage == "internal" and rec.is_magento_source == True:
                    try:
                        ...
                        raise UserError(
                            ("Can not delete source to magento - %s") % tools.ustr(e)
                        )
                return super(StockLocation, self).unlink()
