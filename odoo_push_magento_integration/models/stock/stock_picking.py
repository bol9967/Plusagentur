...
class StockPicking(models.Model):
    ...
    def _create_shipment(self, sale_order_increment_id, client, move_line_ids):
        ...
        for e in move_line_ids:
            if (
                e.location_id.is_magento_source == True
                and e.product_id.default_code in item_id_information
            ):
                if str(e.location_id.id) not in data:
                    if e.product_id.on_magento == True:
                        data[str(e.location_id.id)] = [
                            {
                                "order_item_id": item_id_information[
        ...
        if len(data) > 0:
            for a in data:
                params = {
                    "items": data[a],
                    "arguments": {
                        "extension_attributes": {"source_code": "odoo_location_" + a}
                    },
                }
                url = (
                    "rest/V1/order/" + str(current_magento_order["entity_id"]) + "/ship"
                )
    ...
    def _update_qty_product_source_magento(self, client, move_line_ids):
        for e in move_line_ids:
            ...
            if (
                e.location_id.is_magento_source == True
                and e.product_id.on_magento == True
            ):
            ...
            if (
                e.location_dest_id.is_magento_source == True
                and e.product_id.on_magento == True
            ):
            ...
            if len(data) == 0:
                ...
                try:
                    params = {"sourceItems": data}
                    client.post("rest/V1/inventory/source-items", arguments=params)
                except Exception as e:
                    raise UserError(
                        ("Not create source to magento - %s") % tools.ustr(e)
                    )
        ...
        if self.sale_id.id:
            if magento_backend.prefix_sale_order in self.sale_id.name:
                self._create_shipment(
                    self.sale_id.name.replace(magento_backend.prefix_sale_order, ""),
                    client,
                    self.move_line_ids,
                )
...
class StockScrap(models.Model):
    ...
    def do_scrap(self):
        ...
        client = Client(magento_backend.web_url, magento_backend.access_token, True)
        if (
            self.location_id.is_magento_source == True
            and self.product_id.on_magento == True
        ):
            stock_quant_source_location = request.env["stock.quant"].search(
                [
