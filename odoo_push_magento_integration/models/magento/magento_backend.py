...
class MagentoBackend(models.Model):
    ...
    version = fields.Selection(
        selection=([("Magento 2.3", "2.3"), ("Magento 2.2", "2.2")]),
        default="Magento 2.3",
        string="Version",
        required=True,
    )
    ...
    access_token = fields.Char(
        string="Token", required=True, help="Access token to magento Integration"
    )
    ...
    def pull_magento_backend(self, url, access_token, backend_id):
        try:
            ...
            for website in websites:
                self.env.cr.execute(
                    """INSERT INTO magento_website (name, code,backend_id,external_id,create_date, write_date,create_uid,write_uid)
            ...
            for store_group in store_groups:
                self.env.cr.execute(
                    """INSERT INTO magento_store (name, code, root_category_id, website_id, backend_id, external_id)
                    ...
                    (
                        ...
                        website_odoo_id[
                            website_magento_id.index(store_group["website_id"])
                        ]
                        if check_len_arr
                        else -1,
                        backend_id,
            ...
            for store_view in store_views:
                self.env.cr.execute(
                    """INSERT INTO magento_storeview (name,code,store_id,website_id,is_active,backend_id, external_id)
                    ...
                    (
                        store_view["name"],
                        store_view["code"],
                        client.adapter_magento_id(
                            "magento_store", backend_id, store_view["store_group_id"]
                        ),
                        website_odoo_id[
                            website_magento_id.index(store_view["website_id"])
                        ]
                        if check_len_arr
                        else -1,
                        store_view["is_active"],
                        backend_id,
                        store_view["id"],
                    ),
                )
            ...
            raise UserError(
                _("Not pull data from magento - magento.backend %s") % tools.ustr(e)
            )
