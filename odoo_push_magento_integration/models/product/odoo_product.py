...
class MagentoSyncOdoo(models.Model):
    ...
    type = fields.Selection(
        ...
        ondelete={
            "magento_simple": "cascade",
            "magento_virtual": "cascade",
            "magento_downloadable": "cascade",
            "magento_configurable": "cascade",
            "magento_grouped": "cascade",
            "magento_bundle": "cascade",
        },
    )
    ...
    def create(self, values):
        ...
        if "default_code" in values and values["default_code"] != False:
            count = 0
            self.env.cr.execute(
                """select default_code from product_template WHERE default_code=%s""",
                (values["default_code"],),
            )
            ...
            if len(duplicate_code) > 0:
                raise Warning(
                    _("Code %s Is Duplicate Case With Exist Code!")
                    % values["default_code"]
                )
        if "m2_info_attr_set" in values:
            attr_set_id = (
                self.env["magento.attribute.set"]
                .sudo()
                .search(
                    [("attribute_set_id", "=", values.get("m2_info_attr_set"))], limit=1
                )
                .id
            )
            if attr_set_id:
        ...
        if (
            res.on_magento
            and "m2_info_attr_set" not in values
            and "m2_info_cancel" not in values
        ):
            ...
            if not res.default_code or not res.magento_attr_set:
                raise Warning(
                    _("Need InternalReferenceCode/MagentoSetID For Magento Sync")
                )
            magento = self.env["magento.backend"].search(
                [("id", "!=", False)], limit=1, order="id DESC"
            )
            if magento and res.default_code:
                ...
                if product:
                    ...
                    if res.image_1920:
                        image = res.image_1920.replace(b"\n", b"").decode("ascii")
                        mimetype = self._get_image_type(res.image_1920)
                        body["product"].update(
                            {
                    ...
                    if len(res.product_template_image_ids) > 0:
                        if not res.image_1920:
                            body["product"].update({"media_gallery_entries": []})
                        count = 1
                        for img in res.product_template_image_ids:
                            count += 1
                            if img.image_1920:
                                extra_image = img.image_1920.replace(b"\n", b"").decode(
                                    "ascii"
                                )
                                extra_mimetype = self._get_image_type(img.image_1920)
                    ...
                    custom_attributes = [{"attribute_code": "ac", "value": "4"}]
                    ...
                    body["product"].update({"custom_attributes": custom_attributes})
                    # Call Magento API
                    try:
                    ...
                    try:
                        ...
                        raise Warning(
                            _("Some %s contains incorrect values.Try Again!")
                            % magento_prd.get("message")
                        )
        return res
    ...
    def write(self, values):
        ...
        if "default_code" in values and values["default_code"] != False:
            if values["default_code"] != sku:
                ...
                if len(duplicate_code) > 0:
                    raise Warning(
                        _("Product Code %s Existed!") % values["default_code"]
                    )
        ...
        if "m2_info_cancel" not in values:
            ...
            if self.magento_ok:
                if on_magento:
                    if not (self.type == "product"):
                        raise Warning(
                            _("Need Product Storable For Magento Integration")
                        )
                    if "on_magento" in onchange:
                        ...
                        self.env.cr.execute(
                            """update product_template set magento_ok = FALSE WHERE id=%s""",
                            (self.id,),
                        )
                    else:
                        if "name" in onchange:
                            check += 1
                            body["product"].update({"name": onchange["name"]})
                        if "active" in onchange:
                            ...
                            if onchange["active"] == True:
                                ...
                                active = 0
                            body["product"].update({"status": active})
                        if "list_price" in onchange:
                            check += 1
                            body["product"].update({"price": onchange["list_price"]})
                        if "magento_attr_set" in onchange:
                            check += 1
                            attribute_set_id = (
                                self.env["magento.attribute.set"]
                                .sudo()
                                .search([("id", "=", onchange["magento_attr_set"])])
                                .attribute_set_id
                            )
                            body["product"].update(
                        ...
                        if (
                            "image_medium" in onchange
                            or "product_template_image_ids" in onchange
                        ):
                            check += 1
                            if self.image_1920 == False:
                                body["product"].update({"media_gallery_entries": []})
                            else:
                                image = self.image_1920.replace(b"\n", b"").decode(
                                    "ascii"
                                )
                                mimetype = self._get_image_type(self.image_1920)
                                body["product"].update(
                                    {
                                        "media_gallery_entries": [
                                            {
                            ...
                            if len(self.product_template_image_ids) > 0:
                                position = 1
                                for img in self.product_template_image_ids:
                                    position += 1
                                    if img.image_1920:
                                        extra_image_edit = img.image_1920.replace(
                                            b"\n", b""
                                        ).decode("ascii")
                                        extra_mimetype_edit = self._get_image_type(
                                            img.image_1920
                                        )
                                        body["product"]["media_gallery_entries"].append(
                                            {
                        ...
                        try:
                            if check > 0:
                                ...
                                if a.status_code != 200:
                                    raise Warning(
                                        "Some values contains incorrect values.Try again!"
                                    )
                ...
                if not on_magento:
                    if "on_magento" in onchange:
                        if not self.default_code:
                            raise Warning(
                                _(
                                    "Need InternalReferenceCode/MagentoSetID For Magento Sync"
                                )
                            )
                        else:
                            if not (self.type == "product"):
                                raise Warning(
                                    _("Need Product Storable For Magento Sync")
                                )
                            if self.active == True:
                                active = 1
                            else:
                            ...
                            if len(self.product_template_image_ids) > 0:
                                ...
                                for img in self.product_template_image_ids:
                                    count += 1
                                    extra_image = img.image_1920.replace(
                                        b"\n", b""
                                    ).decode("ascii")
                                    extra_mimetype = self._get_image_type(
                                        img.image_1920
                                    )
                                    body["product"]["media_gallery_entries"].append(
                                        {
                            ...
                            # Call Magento API
                            try:
                                magento_sync = product.update_magento_product(
                                    new_sku, body
                                )
                                # all store magento view
                                ...
                                try:
                                    if magento_sync.json()["id"] is not None:
                                        ...
                                        self.env.cr.execute(
                                            """update product_template set magento_ok = TRUE WHERE id=%s""",
                                            (self.id,),
                                        )
                                except Exception as e:
                                    raise Warning(
                                        _(
                                            "Some %s contains incorrect values.Try Again!"
                                        )
                                        % magento_sync.get("message")
                                        or e
                                    )
                            except Exception as e:
                                raise Warning(
                                    _("Some %s contains incorrect values.") % e
                                )
                # # Call Magento API
            ...
            if not self.magento_ok:
                if "on_magento" in onchange:
                    ...
                    if len(self.product_template_image_ids) > 0:
                        ...
                        for img in self.product_template_image_ids:
                            ...
                            if img.image_1920:
                                ...
                                extra_mimetype = self._get_image_type(self.image_1920)
                                body["product"]["media_gallery_entries"].append(
                                    {
                                        "media_type": "image",
                                        "label": "Image",
                    ...
                    body["product"].update({"custom_attributes": custom_attributes})
                    # Call Magento API
                    magento_sync_after = None
                    try:
                        magento_sync_after = product.update_magento_product(
                            new_sku, body
                        )
                        # All store magento view
                        product.update_magento_product_all(new_sku, body)
                        try:
                            ...
                            if magento_sync_after.status_code not in [200, 201]:
                                message = _(magento_sync_after.text)
                            else:
                                message = e
                            raise Warning(
                                _("Some %s contains incorrect values.Try Again!")
